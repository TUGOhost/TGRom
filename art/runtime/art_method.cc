...
void ArtMethod::SetPackageItem(JNIEnv* env,jobject config){
    LOG(ERROR)<< "tgrom ArtMethod SetPackageItem enter";
    //获取Java中的实例类ParamInfo
    jclass jcInfo = env->FindClass("android/app/PackageItem");
    //获取类中每一个变量的定义
    //boolean boolValue
    ...
    oss << "tgrom SetPackageItem isDeep:"<<packageConfig.isDeep<<" debugMethod:"<<packageConfig.debugMethod<<
...
extern "C" void dumpDexOver()  REQUIRES_SHARED(Locks::mutator_lock_) {
    if(dex_map.size()<=0){
    		LOG(ERROR) << "tgrom dumpDexOver dex_map.size()<=0";
        return;
    }
    char *dexfilepath=(char*)malloc(sizeof(char)*1000);
    LOG(ERROR) << "tgrom ArtMethod::dumpDexOver";
    int result=0;
    ...
    for(iter = dex_map.begin(); iter != dex_map.end(); iter++) {
        ...
        if(dexfilefp>0){
          ...
          if(fp>0)
          {
              ...
              if(result<0)
              {
                  LOG(ERROR) << "tgrom ArtMethod::dumpDexOver,open dexfilepath error";
              }
              fsync(fp);
              close(fp);
              memset(dexfilepath,0,1000);
...
extern "C" void dumpdexfilebyExecute(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
    if(artmethod==nullptr){
        LOG(ERROR)<< "tgrom ArtMethod::dumpdexfilebyExecute artmethod is null";
        return;
    }
    char *dexfilepath=(char*)malloc(sizeof(char)*1000);
    if(dexfilepath==nullptr)
    {
    		std::ostringstream oss;
    		oss<<"tgrom ArtMethod::dumpdexfilebyArtMethod,methodname:"<<artmethod->PrettyMethod().c_str()<<"malloc 1000 byte failed";
        LOG(ERROR)<< oss.str();
        return;
    }
    int result=0;
    ...
    if(dex_file==nullptr){
        LOG(ERROR)<< "tgrom ArtMethod::dumpdexfilebyExecute dex_file is null";
        return;
    }
              ...
              if(fp>0)
              {
                  ...
                  if(classlistfile>0)
                  {
                        for (size_t ii= 0; ii< dex_file->NumClassDefs(); ++ii)
                        {
                            ...
                            if(result<0)
                            {
                                LOG(ERROR) << "tgrom ArtMethod::dumpdexfilebyExecute,write classlistfile file error";
                            }
                            const char* temp="\n";
...
extern "C" void dumpArtMethod(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
            //LOG(ERROR) << "tgrom ArtMethod::dumpArtMethod "<<artmethod->PrettyMethod().c_str();
			char *dexfilepath=(char*)malloc(sizeof(char)*1000);
			if(dexfilepath==nullptr)
			{
				std::ostringstream oss;
				oss<<"tgrom ArtMethod::dumpArtMethodinvoked,methodname:"<<artmethod->PrettyMethod().c_str()<<"malloc 1000 byte failed";
				LOG(ERROR) << oss.str();
				return;
			}
			int result=0;
			char* packageName=ArtMethod::GetPackageName();
				...
				if(dexfilefp>0){
					...
					LOG(ERROR) << "tgrom ArtMethod::dumpdexfilebyArtMethod save dex_map";
				...
				{
							//LOG(ERROR) << "tgrom dumpArtMethodinvoked,methodname:"<<artmethod->PrettyMethod().c_str()<<" write bin";
							int code_item_len = 0;
...
extern "C" void tgextInvoke(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  if (UNLIKELY(__builtin_frame_address(0) < self->GetStackEnd())) {
    ...
    //add
		...
		//				std::ostringstream oss;
		//				oss<<"tgrom art invoke "<<this->PrettyMethod().c_str();
		//				LOG(ERROR) <<oss.str();
		//		}
		//}
	//add end
