...
class ArgArray {
 public:
  ...
  void VarArgsShowArg(const ScopedObjectAccessAlreadyRunnable& soa,
        ...
        REQUIRES_SHARED(Locks::mutator_lock_) {
      std::stringstream ss;
		  ss << "tgrom jni "<<jniMethodName<<"\t"<<methodName<<"\t";
      for (size_t i = 1; i < shorty_len_; ++i) {
        switch (shorty_[i]) {
          case 'Z':
          case 'B':
          ...
          case 'L':{
          	jobject obj=va_arg(ap, jobject);
						...
						if(receiver==nullptr){
							LOG(ERROR)<<"tgrom  "<<jniMethodName<<" receiver =nullptr";
							break;
						}
...
void InvokeWithArgArray(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if(ArtMethod::IsInvokePrint()){
		...
		if(artMethod!= nullptr) {
				...
				oss << "tgrom [InvokeWithArgArray]beforecall caller:" << artMethod->PrettyMethod() << "---called:"<< method->PrettyMethod();
				LOG(ERROR)<<oss.str();
		}
	}
...
void ShowJniField(const char* jniMethodName,jfieldID field) {
		if(ArtMethod::IsJNIMethodPrint()){
			...
			std::ostringstream oss;
			oss << "tgrom jni "<<jniMethodName<<"\t"<<ArtField::PrettyField(f);
			LOG(ERROR)<<oss.str();
		}
  }
...
void ShowArgStr(const char* jniMethod,const char* str1,const char* str2){
	if(ArtMethod::IsJNIMethodPrint()){
			...
			if(str1!=nullptr && str2!=nullptr){
				oss << "tgrom jni "<<jniMethod<<"\t"<<str1<<"\t"<<str2;
			}else if(str1!=nullptr ){
				oss << "tgrom jni "<<jniMethod<<"\t"<<str1;
			}else{
				oss << "tgrom jni "<<jniMethod;
			}
			LOG(ERROR)<<oss.str();
  }
}
