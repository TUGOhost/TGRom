...
//add
extern "C" void tgextInvoke(ArtMethod* artmethod);
...
static jboolean DexFile_settgRomConfig(JNIEnv* env,jclass,jobject config){
    ArtMethod::SetPackageItem(env,config);
    return JNI_TRUE;
}

//addfunction 将ava的Method转换成ArtMethod。然后主动调用
static void DexFile_tgextMethodCode(JNIEnv* env, jclass,jobject method) {
  if(method!=nullptr)
  {
      ArtMethod* proxy_method = jobject2ArtMethod(env, method);
      tgextInvoke(proxy_method);
  }
  return;
}
...
static JNINativeMethod gMethods[] = {
  ...
  //add
  NATIVE_METHOD(DexFile, tgextMethodCode,"(Ljava/lang/Object;)V"),
  NATIVE_METHOD(DexFile, dumpRepair,"()V"),
  NATIVE_METHOD(DexFile, settgRomConfig,"(Ljava/lang/Object;)Z"),
  //add end
};
